...
class SearchPhotoPagingSource(var api: ServiceApi, val query: String) : PagingSource<Int,Results>() {

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Results> {
        val position = params.key ?: 1

        return try {
            val response = api.getPhotosBySearch(query,position, params.loadSize)
